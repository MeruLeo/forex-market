...
export default function ProfilePage({params: { lang }}:{ params: { lang: Locale }}){
    ...
    if(date1 && date2) {
      ...
      const fetchProfile = async () => {
        ...
        try {
          ...
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL2}/profile`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  return (
    <>
      <nav className='fixed h-14 w-full top-0 left-0 z-20 shadow-lg   backdrop-filter backdrop-blur-md' style={{direction: lang === 'en' ? 'ltr': 'rtl'}}>
        <div className='flex h-full justify-between w-full items-center px-4'>
          <div className='object-right pr-4 lg:pr-8 flex'>
            <Link href={`/${lang}/user`} className='w-full text-center flex font-normal items-center justify-center'>
              {dict.profile.home}
      ...
      <div style={{direction: lang === 'en' ? 'ltr': 'rtl'}}  className={`${rubik.className} w-full p-4 min-h-screen pt-20 font-Sahel`}>
      ...
      <div className='px-4 w-full items-center flex gap-2 justify-center mb-2 relative'>
        ...
        <div className='flex flex-col'>
          <div className='flex w-full justify-center items-center'>
            <label htmlFor="" className='mx-2' style={{width:'40px'}}>{dict.profile.from}: </label>
              <DatePicker
                value={date1}
                format="YYYY/MM/DD-HH:mm:ss"
                locale={lang=='en'? gregorian_en :persian_fa }
                calendar={lang=='en'? gregorian :persian }
                plugins={[
                  <TimePicker position="bottom" hStep={2} mStep={3} sStep={4} />,
                ]}
                onChange={setDate1}
                style={{padding:'8px 12px', height:'30px', width: '180px'}}
              />
          </div>
         <div className='flex w-full justify-center items-center'>
          <label htmlFor="" className='mx-2' style={{width:'40px'}}>{dict.profile.to}: </label>
            <DatePicker
              value={date2}
              format="YYYY/MM/DD-HH:mm:ss"
              locale={lang=='en'? gregorian_en :persian_fa }
        ...
        <button className='mr-4 border rounded-md px-2' style={{height:'60px', width:'40px'}} onClick={handleChangeTime}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6">
        ...
        <div className='flex w-full flex-wrap gap-y-2'>
          ...
          <div className='w-full sm:w-1/2 flex gap-2 flex-col'>
            {profile && <ProfileCard profile={profile} dict={dict} />}
          </div>
          <div className='w-full sm:w-1/2 flex gap-2 flex-col'>
            {account && <AccountCard account={account} dict={dict} />}
          </div>

          <div className='w-full  gap-y-3'>
          {transactions && <TransactionsTable transactions={transactions} dict={dict} />}
          </div>
