...
export async function generateMetadata({ params }: any): Promise<Metadata> {
    ...
    const description =
        lang === "en"
            ? "full text of terms and conditions of meta copy"
            : "توضیح کامل شرایط و ضوابط سایت meta copy";
    return {
        title: t,
    ...
    return (
        <div className="flex min-h-screen flex-col items-center overflow-hidden justify-start pb-2 pt-6 px-2">
            <div className="px-5 mb-8 sm:px-10 w-full flex justify-start h-8 items-center">
                <Link href={`/${lang}?signup=1`}>
                    <IoIosArrowRoundBack className="text-3xl hover:text-4xl ease-in-out duration-200 transition" />
                </Link>
            </div>
            ...
            <BlurIn
                word={
                    lang == "en"
                        ? appTerms.en_term_title
                        : lang == "fa"
                          ? appTerms.fa_term_title
                          : appTerms.kur_term_title
                }
