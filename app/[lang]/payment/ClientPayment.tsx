...
const ClientPayment = ({ dict }: { dict: any }) => {
    ...
    const handleNumberChange = (e: any) => {
        ...
        const bankKey = Object.keys(banks).find((key) =>
            rawValue.startsWith(banks[key]),
        );
        if (bankKey) {
        ...
        <main className="flex flex-col items-center w-full min-h-screen md:pt-10 pt-6">
            ...
            <Card className="w-full max-w-96">
                <CardContent
                    ...
                    dir={dict.card.email == "Email" ? "ltr" : "rtl"}
                >
                    <div className="w-full flex flex-col items-center">
                        <label
                            htmlFor="card-number"
                            className="w-full text-xs px-2 translate-y-1"
                        >
                            <span className="bg-white dark:bg-slate-950">
                                {dict.card.number}
                            </span>
                        </label>
                        <Input
                ...
                <CardFooter className="w-full">
                    <div className="flex items-center justify-around px-1 mt-2 w-full">
                        <button
                            className="bg-black text-white dark:bg-white dark:text-black text-base rounded-md px-10 py-[6px] pt-1"
                            // disabled={isLoading}
                            onClick={get_payment}
                        >
                            {dict.card.pay}
