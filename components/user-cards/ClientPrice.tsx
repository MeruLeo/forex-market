...
const ClientPrice = ({ dict, symbols, trades,orders }: { dict: any; symbols: Array<any>; trades: Array<any>; orders: Array<any>; }) => {
  ...
  const [showLeverageIcon, setShowLeverageIcon] = useState(false);
  const [showCopyTradeIcon, setShowCopyradeIcon] = useState(false);
  ...
  return (
    <div className='w-full h-full overflow-hidden'>
      <Card className='w-full h-full'>
        <CardHeader className='flex flex-row items-center justify-center py-2'>
          <div className='flex'>
            {showLeverageIcon?(
                  <SetLeverage dict={dict}  symbols={symbols} />
              ):''}
              {showCopyTradeIcon?(
                  <CopyTrade dict={dict} symbols={symbols} />
              ):''}
          </div>
    ...
    >
      <div className='flex items-center justify-between w-full'>
        <div className='grid grid-cols-2 items-center justify-start w-[56vw] sm:w-auto mr-6'>
          <div className='col-span-1 flex flex-col items-center justify-start w-[23vw] sm:w-auto' style={{ color: '#458bd8' }}>
            <p className='font-bold w-full text-left sm:mx-1'>{item.bid}</p>
            <p className='text-xs w-full text-left sm:mx-1'>{item.low} L</p>
          </div>
          <div className='col-span-1 flex flex-col items-center justify-start w-[23vw] sm:w-auto' style={{ color: '#ee605e' }}>
      ...
      {isOpen && (
        <div className='overflow-hidden transition-all duration-500 ease-in-out w-full mt-0 dark:bg-slate-800 p-1 rounded-md' dir={dict.lang === 'en' ? 'ltr' : 'rtl'}>
          <div className='w-full'>
            ...
            <div className='flex justify-between items-center w-full border-b border-b-gray-500'>
