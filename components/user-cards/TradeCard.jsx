...
const TradeCard = ({ trade, dict, idx }) => {
    ...
    const onCloseTradeClicked = async (ticket, symbol_id, valume) => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/mt5/close_partial`, {
                method: 'POST',
                headers: {
    ...
    return (
        <div key={idx} className='w-full relative min-h-24 flex flex-col justify-center items-center border dark:border-sky-700 m-2 px-3 pb-2 rounded-lg sm:mx-0'>
            ...
            <div className='w-full md:max-w-full md:pr-0 flex items-center justify-between pt-1' onClick={() => setIsOpen(!isOpen)}>
                <p className={`text-base w-14 sm:px-0 font-bold`} style={{ padding: dict.lang==='en' ? '0 8px 0 0' : '0 0 0 8px', color: trade.type === 'buy' ? '#458bd8' : '#ee605e' }}>
              ...
              <p className='space-x-2 w-36 text-sm sm:w-40'>
                    ...
                    <span dir='ltr' style={{ margin: '0 0 0 2px'  }}>
                        <span className='font-semibold text-2xl'>{trade.unit}</span>
                        <span className={`text-xs text-gray-500 ${trade.leverage == 1 ? "hidden" : ""}`}>X</span>
                        <span className={`text-sm ${trade.leverage == 1 ? "hidden" : ""}`}>{trade.leverage}</span>
                    </span>
                </p>
                ...
                <p className='sm:flex text-sm w-14 px-0 font-bold'
                    style={{ color: trade.profit > 0 ? '#458bd8' : '#ee605e' }} >
                    {trade.profit.toFixed(2)}</p>
            </div>
                ...
                <>
                    <div className='w-full flex items-center justify-between mt-2'>
                        <div className="mx-2 sm:mx-0">
                            <IconButton color="inherit" size='small'  disabled={sendingClose} onClick={() => { confirm(dict,dict.confirmation.close_trade, onCloseTradeClicked,1,trade.unit, trade.ticket, trade.symbol_id) }}>
                                <DeleteForeverIcon />
                            </IconButton>
                        </div>
                        <div className='w-full flex items-center justify-center'>
                            <p className=' sm:flex text-sm w-14 px-0 ' style={{direction:"ltr"}}>{dict.lang=='en'? formatDate(trade.time) :trade.time_jalali }</p>
                        </div>
                    ...
                    <div className='w-full flex items-center justify-between'>
                        <p className='space-x-2 w-36 text-sm sm:w-40'>
                            ...
                            <span>:</span>
                            <span dir='ltr'>
                                <span className='font-bold text-[320px]'>{trade.unit}</span>
