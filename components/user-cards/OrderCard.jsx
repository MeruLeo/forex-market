        ...
        <div key={idx} className='w-full relative min-h-24 flex flex-col justify-center items-center border dark:border-sky-700 m-2 px-3 pb-2 rounded-lg'>
            ...
            <div className='w-full flex items-center justify-between pt-1' onClick={() => setIsOpen(!isOpen)}>
                <p className={`space-x-2 w-36 text-sm sm:w-40 font-bold`}>
                    <span className='w-20 inline-block ' style={{padding: dict.lang==='en' ? '0 8px 0 0' : '0 0 0 8px', color: 'orange' }}>
                ...
                <p className='space-x-2 w-36 text-sm sm:w-40'>
                    ...
                    <span dir='ltr' style={{ margin: '0 0 0 2px'  }}>
                        <span className='font-semibold text-2xl'>{order.unit}</span>
                        <span className={`text-xs text-gray-500 ${order.leverage == 1 ? "hidden" : ""}`}>X</span>
                        <span className={`text-sm ${order.leverage == 1 ? "hidden" : ""}`}>{order.leverage}</span>
                    </span>
                </p>
                ...
                <>
                    <div className='w-full flex items-center justify-between mt-2'>
                        <div className="mx-2">
                            ...
                            <IconButton color="inherit" size='small'  disabled={sendingClose} onClick={() => { confirm(dict,dict.confirmation.close_order, onCloseOrderClicked, order.id) }}>
                                <DeleteForeverIcon />
                            </IconButton>
