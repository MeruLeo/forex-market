...
const ClientMessage = ({
    ...
    const handlePageChange = (direction: string) => {
        if (
            direction === "next" &&
            currentPage * itemsPerPage < messages.length
        ) {
            ...
            <Card className="w-full h-full">
                ...
                <CardContent className="w-full px-2">
                    <div className="w-full h-[60vh] sm:h-80 flex flex-col items-center overflow-y-scroll overflow-x-hidden">
                        {getCurrentItems(messages).map(
                            (message: any, idx: number) => (
                                <div
                                    key={idx}
                                    className={`w-full ${message.is_read ? "" : message.is_alert ? "bg-orange-500 animate-pulse" : ""} `}
                                >
                                    <MessageItem
                                        message={message}
            ...
            <div className="w-full flex flex-col items-center justify-center">
                ...
                >
                    ...
                    <DialogContent className="flex flex-col items-center justify-center">
                        ...
                        <div className="flex items-center justify-between px-1 w-full">
                            <p
                                className={`w-full text-sm ${dict_message.lang == "en" ? "text-left" : "text-right"}`}
                                style={{
                                    direction:
                                        dict_message.lang === "en"
                                            ? "ltr"
                                            : "rtl",
                                }}
                            >
                                {["10", "4", "5"].includes(message.code) ? (
                                    message.content
                                        .replaceAll("price", dict_message.price)
                                        .replaceAll("pair", dict_message.pair)
                                ) : message.code === "99" ? (
                                    <>
                                        {content} .
                                        <div
                                        ...
                                        try {
                                            ...
                                            if (dict_message.lang === "kur") {
                                                return (
                                                    ...
                                                    "Default message for Kurdish"
                                                );
                                            } else if (
                                                dict_message.lang === "fa"
                                            ) {
                                                return (
                                                    parsedMessage[
                                                        "message_sender_fa"
                                                    ] ||
                                                    "پیام پیش فرض برای فارسی"
                                                );
                                            } else {
                                                return (
                                                    parsedMessage[
