...
function roundToTwoDecimals(trades: Trade[]): Trade[] {
    return trades.map(trade => ({
        ...trade,  
        commission: trade.commission !== undefined ? parseFloat(trade.commission.toFixed(2)) : trade.commission,
        swap: trade.swap !== undefined ? parseFloat(trade.swap.toFixed(2)) : trade.swap,
        profit: trade.profit !== undefined ? parseFloat(trade.profit.toFixed(2)) : trade.profit,
    }));
}
...
const ClientHistory = ({
    ...
    const handleDateUpdate = (newDate1:string, newDate2:string, newSearch:string)=>{
        setDate1(newDate1);
        setDate2(newDate2);
        setSearch(newSearch)
    }
    ...
    const tradeHeaders = [
        "num", "symbol", "id", "type", "state", "entry", "exit", "profit", 'swap', 'commission', 
        "unit", 'leverage', "tp", "sl", "time", "closeTime",
        ...(accountType.toLowerCase() === "netting" ? ["closed_by"] : [])];


    const [deals, setDeals] = useState([])
    const dealHeaders = [
        "num", "symbol", "ticket", "trade_id", "type", "price", "profit",'commission' , "unit", "time"
    ]

    const [orders, setOrders] = useState([])

    const orderHeaders = [
        "num", "symbol", "id", "type", "price", "unit","leverage", "result", "time"
    ]
    const [transactions, setTransactions] = useState([])
    const transactionsHeaders = [
        "num", "type",  "amount_dollar", "transaction_id", "time",
    ]
        ...
        const get_trades = async () => {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/mt5/user_trades`, {
                    ...
                    body: JSON.stringify({ token: localStorage.getItem('token'), from_date:date1, to_date:date2, search:search }),
                });
                ...
                <CardHeader className='flex items-center justify-center py-2'>
                    <div className='w-full flex items-center justify-between mx-3 mt-3'>
                        <Drawer>
                            ...
                            <Calendar dict={dict} onUpdateDates={handleDateUpdate} />
    ...
    <Table>
      ...
      <TableBody>
        {sortedItems.map((item: any, idx1: number) => (
          <TableRow key={idx1}>
            {headers.map((header: any, idx: number) => (
              <TableCell key={idx} className="font-medium" style={{ color: header === 'profit' ? (item.profit > 0? '#458bd8' : item.profit < 0? '#ee605e': '') :  header === 'type'? (item.type === 'buy'? '#458bd8' : item.type === 'sell'? '#ee605e': item.type === 'deposit'? '#458bd8' : item.type === 'withdraw'? '#ee605e': ''): "" }}>
                {
                  header === 'symbol' ? item.symbol_name[dict.lang] :
                  header === 'num' ? idx1+1 :
                    header === "type" ? isOrder ? dict.history.table[item.type.replaceAll(" ","_")] : dict.history.table[item.type] :
                      header === "dir" ? dict.history.table[item.dir] :
                        header === "result" ? dict.history.table[item.result] :
                        header === "state" ? dict.history.table[item.state] :
                        header === "closeTime" ? dict.lang == 'en' ? formatDate(item.closeTime) : item.closeTime_jalali :
                          header === "time" ? dict.lang == 'en' ? formatDate(item.time) : item.time_jalali :
                            item[header]                
                }
              </TableCell>
            ))}
